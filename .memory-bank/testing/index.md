...
2. Frontend: `pnpm test -- --run` (34 tests as of 2026-03-08)
...
- **Files:** 11 test files / 34 tests
- **Notable coverage:** `src/lib/analytics.test.ts` verifies Yandex goal helper calls and payload shape; `src/lib/components/CurrentQuestion.test.ts` verifies visible special-question category labels and the i18n fallback path; `src/lib/stores/onboarding.test.ts` verifies ordered onboarding persistence, restart behavior, and legacy migration
...
## Known Gaps
- Frontend unit test coverage is still shallower than backend coverage (34 vs 76 tests)
